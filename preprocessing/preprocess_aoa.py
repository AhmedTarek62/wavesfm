...
def parse_args():
    p = argparse.ArgumentParser(description="Preprocess AOA IQ shards into a single HDF5 file with regression targets.")
    p.add_argument("--data-path", required=True, help="Input shard directory or glob (H5 with iq_data/angles).")
    p.add_argument("--output", required=True, help="Destination HDF5 path (e.g., data/aoa_train.h5).")
...
def main():
    ...
    print(f"Wrote AOA HDF5 to {args.output}")
